    ...
    for j in range(n):
        if j != node:
            if j < node:
                ...
                combined_problist.extend(
                    p_rs[j][node] * w_rs[j][node] * edge_mask[j][node])
            else:
                ...
                combined_problist.extend(
                    p_rs[node][j] * w_rs[node][j] * edge_mask[j][node])
...
def normalise(prob, weight, n, bin_dim, seen_list, list_edges, indicator):
    ...
    for i in range(n):
        for j in range(i+1, n):
            ...
            probtemp = np.multiply(np.exp(np.minimum(
                p_rs[i][j] * w_rs[i][j], [10.0, 10.0, 10.0])), np.multiply(indicator[i], indicator[j]))
...
def print_vars(string):
    ...
    print("    "+"\n    ".join(["{} : {}".format(v.name,
                                                 get_shape(v)) for v in tf.get_collection(string)]))
