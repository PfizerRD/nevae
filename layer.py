...
def input_layer(c_mat, adj, weight, feature, k, n, d, activation=None, batch_norm=False, istrain=False, scope=None):
    ...
    w_in = tf.get_variable(name="w_in", shape=[
                           k, d, d], initializer=tf.constant_initializer(0.5))
    w_in = tf.Print(w_in, [w_in], message="my w_in-values:")
    output_list = []
    for i in range(k):
        if i > 0:
            output_list.append(tf.multiply(tf.matmul(feature, w_in[i]), tf.matmul(
                tf.multiply(adj, weight), output_list[i-1])))
        else:
...
def fc_layer(input_, output_size, activation=None, batch_norm=False, istrain=False, scope=None):
    ...
    with tf.variable_scope(scope or "fc"):
        ...
        if batch_norm:
            norm = tf.contrib.layers.batch_norm(tf.matmul(
                input_, w), center=True, scale=True, decay=0.8, is_training=istrain, scope='batch_norm')
            if activation is None:
                return norm
            return activation(norm)
        else:
            b = tf.get_variable(
                name="b", shape=[output_size], initializer=tf.constant_initializer(0.01))
