...
class VAEG(VAEGConfig):
    def __init__(self, hparams, placeholders, num_nodes, num_features, edges, log_fact_k, hde, istest=False):
        ...
        def masked_ll(weight_temp, weight_negative, posscore, posweightscore, temp_pos_score, temp):
            ...
            for i in range(len(self.edges[self.count])):
                ...
                modified_weight = tf.reduce_sum(tf.multiply(np.multiply(
                    indicator[u], indicator[v]), weight_temp[u][v])) / weight_negative[u][v]
                modified_posscore_weighted = modified_weight * \
                    posscore[u][v] * indicator_bridge[u][v] * 1.0

                currentscore = modified_posscore_weighted * \
                    1.0 / (temp_pos_score + temp)
                ll += tf.log(currentscore + 1e-9)

                modified_weight = tf.reduce_sum(tf.multiply(np.multiply(
                    indicator[v], indicator[u]), weight_temp[v][u])) / weight_negative[v][u]
                modified_posscore_weighted = modified_weight * \
                    posscore[v][u] * indicator_bridge[v][u] * 1.0
        ...
        def neg_loglikelihood(prob_dict, w_edge):
            ...
            with tf.variable_scope('NLL'):
                ...
                w_edge_new = tf.exp(tf.minimum(w_edge_new, tf.fill(
                    [self.n, self.n, self.bin_dim], 10.0)))
                ...
                weight_comp = tf.subtract(
                    tf.fill([self.n, self.n], 1.0), w_score)

                dec_mat = tf.exp(tf.minimum(
                    dec_mat_temp, tf.fill([self.n, self.n], 10.0)))
                dec_mat = tf.Print(
                    dec_mat, [dec_mat], message="my decscore values:")

                comp = tf.subtract(
                    tf.ones([self.n, self.n], tf.float32), self.adj)
                comp = tf.Print(comp, [comp], message="my comp values:")

                temp = tf.reduce_sum(tf.multiply(comp, dec_mat))
                negscore = tf.multiply(
                    tf.fill([self.n, self.n], temp+1e-9), weight_comp)
                negscore = tf.Print(
                    negscore, [negscore], message="my negscore values:")

                posscore = tf.multiply(self.adj, dec_mat)
                posscore = tf.Print(
                    posscore, [posscore], message="my posscore values:")

                posweightscore = tf.multiply(posscore, w_score)
                temp_pos_score = tf.reduce_sum(posweightscore)
                posweightscore = tf.Print(
                    posweightscore, [posweightscore], message="my weighted posscore")

                softmax_out = tf.truediv(
                    posweightscore, tf.add(posweightscore, negscore))

                if self.mask_weight:
                    #print("Mask weight option")
                    ll = masked_ll(weight_temp, weight_negative,
                                   posscore, posweightscore, temp_pos_score, temp)
                else:
                    ll = tf.reduce_sum(tf.log(
                        tf.add(tf.multiply(self.adj, softmax_out), tf.fill([self.n, self.n], 1e-9))))
        ...
        def kl_gaussian(mu_1, sigma_1, debug_sigma, mu_2, sigma_2):
            ...
            with tf.variable_scope("kl_gaussisan"):
                ...
                for i in range(self.n):
                    temp_stack.append(
                        tf.matmul(tf.transpose(mu_1[i]),  mu_1[i]))
                ...
                temp_stack = []
                for i in range(self.n):
                    temp_stack.append(tf.reduce_prod(
                        tf.square(debug_sigma[i])))
                third_term = tf.log(
        ...
        def get_lossfunc(enc_mu, enc_sigma, debug_sigma, prior_mu, prior_sigma, dec_out, w_edge, label):
            # KL_divergence loss
            kl_loss = kl_gaussian(
                enc_mu, enc_sigma, debug_sigma, prior_mu, prior_sigma)
            likelihood_loss = neg_loglikelihood(
                dec_out, w_edge)  # Cross entropy loss
        ...
        self.adj = tf.placeholder(dtype=tf.float32, shape=[
                                  self.n, self.n], name='adj')
        self.features = tf.placeholder(dtype=tf.float32, shape=[
                                       self.n, self.d], name='features')
        self.weight = tf.placeholder(dtype=tf.float32, shape=[
                                     self.n, self.n], name="weight")
        self.weight_bin = tf.placeholder(dtype=tf.float32, shape=[
                                         self.n, self.n, hparams.bin_dim], name="weight_bin")
        self.input_data = tf.placeholder(dtype=tf.float32, shape=[
                                         self.k, self.n, self.d], name='input')
        self.eps = tf.placeholder(dtype=tf.float32, shape=[
                                  self.n, self.z_dim, 1], name='eps')

        self.cell = VAEGCell(self.adj, self.weight,
                             self.features, self.z_dim, self.bin_dim)
        ...
        self.cost = get_lossfunc(
            enc_mu, enc_sigma, debug_sigma, prior_mu, prior_sigma, dec_out, w_edge, label)
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(len(adj)):
                ...
                feed_dict = construct_feed_dict(
                    lr, dr, self.k, self.n, self.d, decay, placeholders)
                ...
                input_, train_loss, _, probdict, cx, w_edge = self.sess.run(
                    [self.input_data, self.cost, self.apply_transform_op, self.prob, self.c_x, self.w_edge], feed_dict=feed_dict)
    ...
    def getembeddings(self, hparams, placeholders, adj, deg, weight_bin, weight):
        ...
        prob, ll, kl, w_edge, embedding = self.sess.run(
            [self.prob, self.ll, self.kl, self.w_edge, self.z_encoded], feed_dict=feed_dict)
        return embedding
    ...
    def sample_graph_slerp(self, hparams, placeholders, s_num, G_good, G_bad, inter, ratio, index, num=10):
        ...
        for i in range(self.n):
            ...
            if i == index:
                if inter == "lerp":
                    new_graph.append(
                        lerp(np.reshape(node_good, -1), np.reshape(node_bad, -1), ratio))
                else:
                    new_graph.append(
                        slerp(np.reshape(node_good, -1), np.reshape(node_bad, -1), ratio))
            else:
        ...
        p, list_edges, w_new = normalise(
            prob, w_edge, self.n, self.bin_dim, [], list_edges, indicator)
        ...
        for i in range(hparams.edges - 1):
            ...
            p, list_edges, w_new = normalise(
                prob, w_edge, self.n, self.bin_dim, candidate_edges, list_edges, indicator)
            candidate_edges.extend([list_edges[k] for k in np.random.choice(
                range(len(list_edges)), [1], p=p, replace=False)])
                ...
                if node in hn:
                    for i1 in range(self.n):
                        if hydro_sat[i1] == node_list[i1] - 1:
                            ...
                            edge_mask[node][i1] = 0
                while d < deg_req:
                    p = normalise_h1(prob, w_edge,  self.bin_dim,
                                     indicator, edge_mask, node)
                    #print("Debug p", p)
        ...
        while (count < 1):
            ...
            for i1 in range(num_edges - 1):
                ...
                try:
                    ...
                    #candidate_edges = []
                    continue
            structure_list[' '.join([str(u) + '-'+str(v)+'-'+str(w)
                                     for (u, v, w) in sorted(candidate_edges)])] += 1
            count += 1
    ...
    def get_unmasked_candidate(self, list_edges, prob, w_edge, num_edges):
        ...
        while (count < 50):
            ...
            structure_list[' '.join([str(u) + '-'+str(v)+'-'+str(w)
                                     for (u, v, w) in sorted(candidate_edges, key=itemgetter(0))])] += 1
        ...
        for i in range(len(adj)):
            ...
            for j in [1]:
                ...
                with open(hparams.z_dir+'weight_mat'+str(i)+'.txt', 'a') as f:
                    for x in range(self.n):
                        f.write(
                            '['+','.join([str(el[0])+' '+str(el[1])+' '+str(el[2]) for el in w_edge[x]])+']\n')
                    f.write("\n")
                ...
                if not hparams.mask_weight:
                    ...
                    print("Mask")
                    (atom_list, valency_arr) = self.getatoms(
                        hparams.nodes, labels)
                    candidate_edges = self.get_masked_candidate_with_atom_ratio_new(
    ...
    def sample_graph(self, hparams, placeholders, adj, features, weights, weight_bins, s_num, node, hde, num=10, outdir=None):
        ...
        prob, ll, z_encoded, kl, sample_mu, sample_sigma, loss, w_edge, labels = self.sess.run(
            [self.prob, self.ll, self.z_encoded, self.kl, self.enc_mu, self.enc_sigma, self.cost, self.w_edge, self.label], feed_dict=feed_dict)
        prob = np.reshape(prob, (self.n, self.n))
        w_edge = np.reshape(w_edge, (self.n, self.n, self.bin_dim))
        ...
        if not hparams.mask_weight:
            ...
            while trial < 5000:
                candidate_edges = [list_edges[i] for i in np.random.choice(
                    range(len(list_edges)), [hparams.edges], p=p, replace=False)]
