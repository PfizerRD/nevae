...
class VAEGCell(object):
    ...
    def __call__(self, c_x, n, d, k, eps_passed, sample, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            c_x = input_layer(c_x, self.adj, self.weight, self.features, k, n,
                              d, activation=None, batch_norm=False, istrain=False, scope=None)
            c_x = tf.Print(c_x, [c_x], message="my c_x-values:")

            with tf.variable_scope("Prior"):
                prior_mu = tf.zeros(shape=[n, self.z_dim, 1], name="prior_mu")
                prior_sigma = tf.matrix_diag(
                    tf.ones(shape=[n, self.z_dim]), name="prior_sigma")

            with tf.variable_scope("Encoder"):
                list_cx = tf.unstack(c_x)
                # output will be of shape n X kd
                enc_hidden = fc_layer(
                    tf.concat(list_cx, 1), k*d, activation=tf.nn.relu, scope="hidden")
                # output will be of shape n X 5 (this is a hyper paramater)
                enc_mu = fc_layer(enc_hidden, self.z_dim,
                                  activation=tf.nn.softplus, scope='mu')
                enc_mu = tf.reshape(enc_mu, [n, self.z_dim, 1])
                enc_mu = tf.Print(enc_mu, [enc_mu],
                                  message="my enc_mu-values:")

                # output will be n X 1 then convert that to a diagonal matrix
                debug_sigma = fc_layer(
                    enc_hidden, self.z_dim, activation=tf.nn.softplus, scope='sigma')
                debug_sigma = tf.Print(
                    debug_sigma, [debug_sigma], message="my debug_sigma-values:")
                enc_sigma = tf.matrix_diag(debug_sigma, name="enc_sigma")
                enc_sigma = tf.Print(
                    enc_sigma, [enc_sigma], message="my enc_sigma-values:")
            ...
            with tf.variable_scope("Decoder"):
                ...
                for u in range(n):
                    ...
                    for j in range(4):
                        ...
                        z_stack_label.append(
                            tf.concat(values=(tf.transpose(z[u]), m), axis=1)[0])
                    #'''
                    for v in range(n):
                        z_stack.append(
                ...
                dec_hidden = fc_layer(
                    tf.stack(z_stack), 1, activation=tf.nn.softplus, scope="hidden")
                weight = fc_layer(tf.stack(z_stack_weight), 1,
                                  activation=tf.nn.softplus, scope="marker")
                label = fc_layer(tf.stack(z_stack_label), 1,
                                 activation=tf.nn.softplus, scope="label")
